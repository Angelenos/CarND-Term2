...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars) {
        ...
        for (int t = 0; t < N; t++) {
            fg[0] += Kref * CppAD::pow(vars[cte_start + t], 2);
            fg[0] += Kref * CppAD::pow(vars[epsi_start + t], 2);
            fg[0] += 0.001 * Kref * CppAD::pow(vars[v_start + t] - ref_v, 2);
        }
        ...
        for (int t = 1; t < N; t++) {
            ...
            fg[1 + epsi_start + t] = epsi1 - (dpsi - v0 * delta0 / Lf * dt);
        }
    }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    for (int i = a_start; i < n_vars; i++) {
        vars_lowerbound[i] = -15.0;
        vars_upperbound[i] = 15.0;
    }
    ...
    cout << "Optimization success? " << (ok &= solution.status == CppAD::ipopt::solve_result<Dvector>::success) << endl;
